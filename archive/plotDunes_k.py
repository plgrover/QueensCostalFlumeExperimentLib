...
def readTurbLineData(filePath):
    fil = open(filePath,'r')
    retval = ULine()
    for line in fil:
      row = [x.strip(' \t\n\r') for x in line.split(' \t')]
      retval.y.append(float(row[0]))
      retval.k.append(float(row[1]))
      retval.nut.append(float(row[2]))
      retval.omega.append(float(row[3]))
...
def main(path, id):
  ...
  for profile in xrange(1,13,1):
    filePathTemplate='{0}/postProcessing/sets/{1}/Dune{2}_{3}_k_nut_omega.xy'.format(path, id, 6, profile)  
    modelResult = readTurbLineData(filePathTemplate)
